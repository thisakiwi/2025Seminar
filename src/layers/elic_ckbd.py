...
from src.entropy_models import GaussianConditional, EntropyModel
...
def ckbd_nonanchor2(y):
    nonanchor = torch.zeros_like(y).to(y.device)
    nonanchor[:, :, 0::2, 1::2] = y[:, :, 0::2, 1::2]
    return nonanchor

def ckbd_nonanchor3(y):
    nonanchor = torch.zeros_like(y).to(y.device)
    nonanchor[:, :, 1::2, 0::2] = y[:, :, 1::2, 0::2]
    return nonanchor

def ckbd_nonanchor4(y):
    nonanchor = torch.zeros_like(y).to(y.device)
    nonanchor[:, :, 1::2, 1::2] = y[:, :, 1::2, 1::2]
    return nonanchor

def ckbd_anchor1_sequeeze(y):
    anchor = y[:, :, 0::2, 0::2]
    return anchor

def ckbd_nonanchor2_sequeeze(y):
    anchor = y[:, :, 0::2, 1::2]
    return anchor

def ckbd_nonanchor3_sequeeze(y):
    anchor = y[:, :, 1::2, 0::2]
    return anchor

def ckbd_nonanchor4_sequeeze(y):
    anchor = y[:, :, 1::2, 1::2]
    return anchor